...
def runPolicies(full_traj, 
	...
	m  = MiceNNModel(int(args.options), statedim=(6*N_FRAMES, 1), actiondim = (6*N_FRAMES_AFTER, 1)
...
def main():
	...
	for i in range(int(args.ntrajs)):
		...
		key = key_list[i]
		pointtraj = hf.get(key+ '/points')[()][:, :3, :]     # 240,7,2
		conftraj = hf.get(key+ '/confidence')[()]	
		velocitytraj = hf.get(key+ '/velocity')[()][:, :3, :]	# 239,7,2
		print('Points', velocitytraj.shape)
	...
	runPolicies(full_traj, 5*np.eye(6* N_FRAMES_AFTER), n_start_states = 5) #np.load('/projects/kumar-lab/mehtav/cov.npy').astype(np.float32), file_name = 'LR1e0')
