...
policy_networks = []
sess = tf.Session()
new_saver = tf.train.import_meta_graph(args.model + '/'+ args.model + '.meta')
new_saver.restore(sess, tf.train.latest_checkpoint(args.model+ '/'))
...
def restore_network(sdim, 
			 ...
			 cov_mat = np.eye(14*N_FRAMES_AFTER),
			 MLPcounter = 0):
	"""
...
def visualizePolicy(option_no, start_state, frames = 600, filename=None, trajectory_only = False):
	...
	for i in range(frames):
		...
		feed_dict[policy_networks[option_no]['action']] = np.reshape(np.zeros(14*N_FRAMES_AFTER), (1, N_FRAMES_AFTER*14)).astype(np.float32)
		...
		next_frame = curr_frame + reshaped_a[0]/30/PX_TO_CM

		print('Action: ')
		print(a.shape, curr_frame.shape)
		vis_traj.append((curr_frame, np.reshape(reshaped_a[0], (14))))
