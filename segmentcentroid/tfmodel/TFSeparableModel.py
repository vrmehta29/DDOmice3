...
class TFSeparableModel(TFModel):
    ...
    def getPolicyLossFunction(self):
        ...
        return tf.reduce_sum(loss_array), pi_vars, loss_array


    def getTransLossFucntion(self):
        ...
        return tf.reduce_sum(loss_array), psi_vars, loss_array
    ...
    def initialize_restore(self):
        """
        Initializes the internal state
        """

        for i in range(0, self.k):
            self.policy_networks.append(self.restore_network())

        for i in range(0, self.k):
            self.transition_networks.append(self.restore_trans_network())
