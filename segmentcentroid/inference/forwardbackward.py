...
class ForwardBackward(object):
    ...
    def init_iter(self, index, X, tabulate=True):
        ...
        self.Q = np.ones((len(X)+1, self.k) , dtype='float64')/self.k
        self.fq = np.zeros((len(X)+1, self.k) , dtype='float64')
        self.bq = np.zeros((len(X)+1, self.k), dtype='float64')
        self.B = np.zeros((len(X)+1, self.k), dtype='float64')
